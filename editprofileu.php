...
<?php
  ...
  if (isset($_POST['submit'])) 
  {
    ...
    if (!$error) {
      if (!empty($first_name) && !empty($last_name) && !empty($gender) && !empty($joindate) && !empty($city) && !empty($state)) {
        // Only set the picture column if there is a new picture
        if (!empty($new_picture)) {
          $query = "UPDATE mismatch_user SET first_name = '$first_name', last_name = '$last_name', gender = '$gender', " .
            " joindate = '$joindate', city = '$city', state = '$state', picture = '$new_picture' WHERE user_id = '" . $_SESSION['user_id'] . "'";
        }
        else {
          $query = "UPDATE userr SET firstname = '$firstname', lastname = '$lastname', gender = '$gender', " .
            " joindate = '$joindate', city = '$city', state = '$state' WHERE user_id = '" . $_SESSION['user_id'] . "'";
        }
    ...
    $query = "SELECT firstname, lastname, gender, joindate, city, country, picture FROM user WHERE user_id = '" . $_SESSION['user_id'] . "'";
...
?>
  ...
  <form enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    ...
    <fieldset>
      ...
      <input type="text" id="joindate" name="joindate" value="<?php if (!empty($joindate)) echo $joindate; else echo 'YYYY-MM-DD'; ?>" /><br />
